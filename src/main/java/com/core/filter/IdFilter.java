...
//		response.addCookie(cookie);
        if (requestPath.matches(".*css") || requestPath.matches(".*js") || requestPath.matches(".*assets.*")) {
            System.out.println("doFilter-1區:css&js區");
            chain.doFilter(request, response);
                ...
                ||requestPath.matches(".*cart.*")
        ) {
            if (requestPath.equals("/adServlet")) {
                System.out.println("doFilter-2區:廣告區");
                chain.doFilter(request, response);
                return;
            }
            System.out.println("doFilter-3區:免登入區");
            if (mId >= 1) {
                chain.doFilter(request, response);
                return;
            }
            ...
            System.out.println("doFilter-4區:Servlet跳轉區");
            chain.doFilter(request, response);
            ...
            System.out.println("doFilter-5區:後台登入區");
            chain.doFilter(request, response);
        } else if (((requestPath.matches(".*/front-end/Member/.*")||requestPath.matches(".*/cart/add.*")||requestPath.matches(".*Write.*")||requestPath.matches(".*order.do.*")
                ||requestPath.matches(".*orderDetail.do.*")) && mId >= 1)) {
            System.out.println("doFilter-6區:會員登入區");
            chain.doFilter(request, response);
        } else if ((((requestPath.matches(".*/front-end/store/.*") || requestPath.matches(".*goods.do.*")) && sId >= 1 && storeplan >= 1)) ||
                ((requestPath.matches(".*StoreStart_blank.*") && sId >= 1 && storeplan >= 1))) {
            System.out.println("doFilter-7區:店家登入區");
            ...
            System.out.println("doFilter-8區:後台區");
            chain.doFilter(request, response);
        } else if (requestPath.matches(".*/CGA105G2/")) {
            System.out.println("doFilter-9區:首頁區");
            chain.doFilter(request, response);
        } else {
